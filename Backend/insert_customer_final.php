...
$r_id=pg_query($db_connection,"Select e_id from employee where r_id in (Select r_id from restaurant where name='$restaurant')");
$c_id=uniqid('C');
$row = pg_fetch_row($r_id);
$result_r_id=$row[0];

$result=pg_query($db_connection,"Insert into customer Values('$c_id','$name','$phone','$zip','$result_r_id')");
