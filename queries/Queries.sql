\c restaurant_finder;
select * from restaurant where closing_time >='23:30';

select * from restaurant where opening_time>='8:00' and closing_time >='23:30';

select cname from customer where e_id in('E55','E46','E68');

select cname from customer where e_id='E55';
...
select Name from restaurant,restaurant_address where restaurant.R_id=restaurant_address.R_id and Street='Esplanade';

select E_Name from employee,restaurant where E_id in(select E_id from employee_contact group by E_id having count(*)>=2) and employee.R_id=restaurant.R_id and restaurant.Name='Absolute Barbecues';
